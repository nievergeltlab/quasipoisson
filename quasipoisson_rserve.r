#source('dzinb_pois_cpp2_working_rplink.r')
library(extraDistr)
library(Rcpp)
...
Rplink <- function(PHENO, GENO, CLUSTER, COVAR)
{
    ...
    subid <- seq(1, length(PHENO))
       ...
       COVAR_mat <- as.matrix(cbind(1,s,COVAR)) # the 1 is for the intercept!!
       
       #just make some junk names for the other covariates
       #names(COVAR_mat)[2:ncol(COVAR_mat)] <- paste("covar_",2:ncol(COVAR_mat),sep="")
       #print(head(COVAR_mat))
 ...
 #NOTICE that this tosses out the trauma count as a covariate but keeps it as a weight
 
 
qp.fit <- try( quasipoislm(y = PHENO ,xx =  COVAR_mat, ntries = 3,method = "BFGS"),
               silent = TRUE)
	               
        if(class(qp.fit) !=  "try-error")
        {
                #Get the Beta, SE, z stat
                #coefres <- summary(gee.fit)$coefficients[1,]
		              r <- c(qp.fit$coefficients[2],qp.fit$se[2],qp.fit$coefficients[2]/qp.fit$se[2],2*pnorm(abs(qp.fit$coefficients[2]/qp.fit$se[2]),lower.tail=F))
        } else {
